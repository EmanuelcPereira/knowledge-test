...
describe('DeletePurchaseOrders Controller', () => {
    ...
    it('should return 400 if PurchaseOrderRepository findOne() throws', async () => {
        const { sut, purchaseOrderRepositorySpy } = makeSut();
        jest.spyOn(purchaseOrderRepositorySpy, 'findOne').mockReturnValueOnce([]);
        const httpResponse = await sut.handle(mockRequest());
        expect(httpResponse).toEqual(badRequest('Id not found'));
    });
